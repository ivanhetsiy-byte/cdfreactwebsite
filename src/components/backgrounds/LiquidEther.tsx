...
// Vendored from React Bits (LiquidEther).
// @ts-nocheck
...
export default function LiquidEther({
 ...
 arr = stops.length === 1 ? [stops[0]!, stops[0]!] : (stops as string[]);
 } else {
 arr = ['#ffffff', '#ffffff'];
 }
 const w = arr.length;
 ...
 /** Hero (or mount) only — never the window, so off-hero moves are ignored. */
 listenerTarget: HTMLElement | null = null;
 isHoverInside = false;
 ...
 private _onPointerLeave = this.onDocumentLeave.bind(this);
 init(container: HTMLElement) {
 this.container = container;
 this.docTarget = container.ownerDocument || null;
 // Prefer the wrapping <section> so interaction matches the hero bounds,
 // not the full window (and not just the WebGL mount).
 this.listenerTarget =
 ...
 this.listenerTarget.addEventListener("mouseleave", this._onPointerLeave);
 this.listenerTarget.addEventListener("touchcancel", this._onPointerLeave);
 }
 dispose() {
 if (this.listenerTarget) {
   ...
   this.listenerTarget.removeEventListener("mouseleave", this._onPointerLeave);
   this.listenerTarget.removeEventListener("touchcancel", this._onPointerLeave);
 }

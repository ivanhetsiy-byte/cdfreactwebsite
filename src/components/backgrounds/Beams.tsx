...
/** Vendored from React Bits Beams — crossing animated ribbons (R3F). */
export function Beams({
  ...
  return (
    <div className={`h-full w-full ${className}`.trim()} aria-hidden="true">
      <CanvasWrapper>
...
>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    if (!mesh.current?.material?.uniforms?.time) return;

...
/** Figma frame 64:6 — 1422 × 1247; child coords offset from frame origin (40, 850). */
const FRAME_W = 1422;
const FRAME_H = 1247;
...
/** Thin faded grid lines from Figma vectors 62:70–62:98 (HTML dividers, not borders). */
function GridLine({
  orientation,
  left,
  top,
...
/**
 * Brush-stroke underline — exact vector path from Figma node 62:104.
 * Path data sourced from the MCP SVG export of that node.
 * Anchored locally under its parent text frame (not the section canvas).
 */
function GrowsUnderline() {
  return (
    <svg
      viewBox="0 0 601.148 14.289"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="absolute left-0 bottom-[-8px] w-full h-auto pointer-events-none text-black dark:text-white"
      aria-hidden="true"
...
export function MissionStatement() {
  ...
  useEffect(() => {
    ...
    // Document-root Lenis scroll — observe against the viewport (root: null)
  ...
  return (
    <section
      ...
      className="w-full relative overflow-hidden bg-white text-black dark:bg-black dark:text-white swiss-no-select pt-32 pb-24 md:pt-44 md:pb-32"
    >
      {/* Proportion box locks Figma 1422×1247 spatial relationships */}
      <div
        className="relative mx-auto w-full max-w-[1422px]"
        style={{ aspectRatio: `${FRAME_W} / ${FRAME_H}` }}
      >
        {/* 62:136 — Header (translates with language) */}
        <p
        ...
        {/* 62:270 — Diamond-masked dancer (333×512 @ 1107,850 → relative 1067,0)
            Figma 62:195 IMAGE CROP: transform [[0.4338,0,0.2171],[0,1,~0]]
            → object-position X = tx/(1-sx) ≈ 38.34% (content shifted right in-frame) */}
        <div
          ...
          <Image
            ...
            className={`object-cover object-[38.34%_100%] filter cursor-pointer will-change-transform transform-gpu backface-hidden [will-change:transform,filter] ${
              isElementInView
                ...
                : "md:grayscale md:hover:grayscale-0 grayscale brightness-95 contrast-105 md:hover:brightness-100 md:hover:contrast-100"
            }`}
          />
        </div>

        {/* Faded grid — vertical (62:70, 62:94, 62:71) */}
        ...
        {/* Faded grid — horizontal; 62:96+62:98 leave a gap through the diamond */}
        <GridLine orientation="horizontal" left={79} top={512} length={915} />
        <GridLine orientation="horizontal" left={1161} top={512} length={261} />
        <GridLine orientation="horizontal" left={224} top={795} length={997} />
        <GridLine orientation="horizontal" left={415} top={1060} length={997} />

        {/* 62:84 — Professional Pedagogy (rotated −90°) */}
        <div
        ...
        {/* Staircase headline — locked English Helvetica (never translated) */}
        <h2 id="mission-heading" className="sr-only">
          {MISSION_LOCKED_TITLE.full}
        </h2>
        <p
          aria-hidden="true"
          className="absolute font-swiss text-[8vw] md:text-[10vw] font-black leading-none tracking-tighter text-black dark:text-white whitespace-nowrap"
        ...
        <p
          aria-hidden="true"
          className="absolute font-swiss text-[8vw] md:text-[10vw] font-black italic leading-none tracking-tighter text-black dark:text-white whitespace-nowrap"
          style={{ left: pct(846, FRAME_W), top: pct(795, FRAME_H) }}
        >
          <span className="relative inline-block whitespace-nowrap">
            {MISSION_LOCKED_TITLE.grows}
            {/* 62:104 — Brush underline pinned under Grows */}
            <GrowsUnderline />
          </span>
        </p>

        {/* 62:87 — Subheading (locked English: "Dance for all") */}
        <p
